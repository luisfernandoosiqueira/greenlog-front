...
export class ConexaoService {
  ...
  findAll(): Observable<RuaResponse[]> {
    return this.http
      .get<RuaResponse[]>(this.apiUrl)
      .pipe(catchError(this.handleError));
  }

  findById(id: number): Observable<RuaResponse[]> {
    return this.http
      .get<RuaResponse[]>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  findByOrigemId(origemId: number): Observable<RuaResponse[]> {
    return this.http
      .get<RuaResponse[]>(`${this.apiUrl}/origem/${origemId}`)
      .pipe(catchError(this.handleError));
  }

  findByDestinoId(destinoId: number): Observable<RuaResponse[]> {
    return this.http
      .get<RuaResponse[]>(`${this.apiUrl}/origem/${destinoId}`)
      .pipe(catchError(this.handleError));
  }

  create(novaConexao: RuaRequest): Observable<RuaResponse> {
    return this.http
      .post<RuaResponse>(this.apiUrl, novaConexao)
      .pipe(catchError(this.handleError));
  }

  update(atualizarConexao: RuaRequest, id: number): Observable<RuaResponse> {
    return this.http
      .put<RuaResponse>(`${this.apiUrl}/${id}`, atualizarConexao)
      .pipe(catchError(this.handleError));
  }

  // mantém o erro original para que o componente consiga ler a mensagem do backend
