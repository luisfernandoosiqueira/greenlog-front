...
export class ItinerarioService {
  ...
  findAll(): Observable<ItinerarioResponse[]> {
    return this.http
      .get<ItinerarioResponse[]>(this.apiUrl)
      .pipe(catchError(this.handleError));
  }

  findByData(id: number): Observable<ItinerarioResponse> {
    return this.http
      .get<ItinerarioResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  create(novoItinerario: ItinerarioRequest): Observable<ItinerarioResponse> {
    return this.http
      .post<ItinerarioResponse>(this.apiUrl, novoItinerario)
      .pipe(catchError(this.handleError));
  }

  update(atualizarItinerario: ItinerarioRequest, id: number): Observable<ItinerarioResponse> {
    return this.http
      .put<ItinerarioResponse>(`${this.apiUrl}/${id}`, atualizarItinerario)
      .pipe(catchError(this.handleError));
  }

  delete(id: number): Observable<ItinerarioResponse> {
    return this.http
      .delete<ItinerarioResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  // propaga o erro original para que o componente possa ler a mensagem do backend
  private handleError(error: HttpErrorResponse) {
    console.error('Erro na API de itinerários: ', error);
