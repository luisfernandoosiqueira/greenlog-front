...
export class MotoristaService {
  ...
  findAll(): Observable<MotoristaResponse[]> {
    return this.http
      .get<MotoristaResponse[]>(this.apiUrl)
      .pipe(catchError(this.handleError));
  }

  findByCpf(cpf: string): Observable<MotoristaResponse> {
    return this.http
      .get<MotoristaResponse>(`${this.apiUrl}/${cpf}`)
      .pipe(catchError(this.handleError));
  }

  create(novoMotorista: MotoristaRequest): Observable<MotoristaResponse> {
    return this.http
      .post<MotoristaResponse>(this.apiUrl, novoMotorista)
      .pipe(catchError(this.handleError));
  }

  update(atualizarMotorista: MotoristaRequest, cpf: string): Observable<MotoristaResponse> {
    return this.http
      .put<MotoristaResponse>(`${this.apiUrl}/${cpf}`, atualizarMotorista)
      .pipe(catchError(this.handleError));
  }

  delete(cpf: string): Observable<MotoristaResponse> {
    return this.http
      .delete<MotoristaResponse>(`${this.apiUrl}/${cpf}`)
      .pipe(catchError(this.handleError));
  }

  // repassa o erro original para o componente tratar a mensagem do backend
