...
export class PontoColetaService {
  ...
  findAll(): Observable<PontoColetaResponse[]> {
    return this.http
      .get<PontoColetaResponse[]>(this.apiUrl)
      .pipe(catchError(this.handleError));
  }

  findById(id: number): Observable<PontoColetaResponse> {
    return this.http
      .get<PontoColetaResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  create(novoPonto: PontoColetaRequest): Observable<PontoColetaResponse> {
    return this.http
      .post<PontoColetaResponse>(this.apiUrl, novoPonto)
      .pipe(catchError(this.handleError));
  }

  update(atualizarPonto: PontoColetaRequest, id: number): Observable<PontoColetaResponse> {
    return this.http
      .put<PontoColetaResponse>(`${this.apiUrl}/${id}`, atualizarPonto)
      .pipe(catchError(this.handleError));
  }

  delete(id: number): Observable<PontoColetaResponse> {
    return this.http
      .delete<PontoColetaResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  // repassa o erro original para permitir exibir a mensagem do backend
  private handleError(error: HttpErrorResponse) {
    console.error('Erro na API de Pontos de Coleta: ', error);
